...
class ItemController extends RubricAwareController
{
    ...
    public function siteIndexAction(Request $request)
    {
        ...
        $filter_params['SizeFilter'] = isset($request->query->get('ItemsFilter')['size']) ? $request->query->get(
            'ItemsFilter'
        )['size'] : null;
    ...
    public function addCommonAction()
    {
        ...
        if (!is_object($user)) {
            ...
            return $this->render(
                'TooBigAppBundle:Rubric:select_rubric.html.twig',
                [
                    'rubric' => $rubric,
                ]
            );
    ...
    public function listRubricChildrenAction(Request $request)
    {
        ...
        if (null !== $rubric) {
            ...
            return $this->render(
                'TooBigAppBundle:Rubric:rubric_children.html.twig',
                [
                    'rubric' => $rubric,
    ...
    public function addAction($rubric_id, Request $request)
    {
        ...
        if (!is_object($user)) {
            ...
            if (!preg_match('/^\d+$/', $editId)) {
                ...
                if ($record->getId()) {
                    $this->get('punk_ave.file_uploader')->syncFiles(
                        array(
                            'from_folder' => 'attachments/'.$record->getId(),
                ...
                if ($form->isValid()) {
                    try {
                        ...
                        $fileUploader->syncFiles(
                            array(
                                'from_folder' => '/tmp/attachments/'.$editId,
            ...
            $existingFiles = $this->get('punk_ave.file_uploader')->getFiles(
                array('folder' => 'tmp/attachments/'.$editId)
            );

            return $this->render(
                'TooBigAppBundle:Item:add_item.html.twig',
                [
                    'rubric' => $rubric,
        ...
        if (!is_object($user)) {
            ...
            if ($user === $record->getCreatedBy()) {
                    ...
                    if ($form->isValid() && $form->isSubmitted()) {
                        try {
                            ...
                            try {
                                ...
                                foreach ($files as $key => $file_name) {
                                    if (!is_object(
                                        $this->get('blueimp_model')->getFileByItemName($record, $file_name)
                                    )
                                    ) {
                                        $this->get('blueimp_model')->createFile($record, $file_name);
                                    }
                            ...
                            $this->get('flash_bag')->addMessage(
                                'Произошла ошибка при сохранении объявления! '.$e->getMessage()
                            );
                        }
                    } else {
                        $this->get('flash_bag')->addMessage(
                            'Произошла ошибка при сохранении объявления! Ошибка валидации формы! '.$form->getErrors(
                            )->next()
                        );
                    }
                }
                ...
                if (count($existingFiles) === 0) {
                    $existingFiles = $this->get('punk_ave.file_uploader')->getFiles(
                        array('folder' => 'attachments/'.$record->getId())
                    );
                }

                return $this->render(
                    'TooBigAppBundle:Item:edit_item.html.twig',
                    [
                        'rubric' => $rubric,
                        'form' => $form->createView(),
                        'posting' => $record,
                ...
                $this->get('flash_bag')->addMessage(
                    '<div>Данное объявление создано не вами. Желаете создать объявление на основе текущего?</div><div class="form-group control-group"><div class="col-sm-12 form-buttons center"><a class="btn btn-success" href="'.$this->generateUrl(
                        'app_item_copy',
                        ['item_id' => $record->getId()]
                    ).'">Да</a><a class="btn btn-warning" href="'.$rubric->getFullPath().$record->getSlug(
                    ).'">Нет</a></div></div>'
                );
    ...
    public function watchStatusAction($item_id)
    {
        ...
        if (!$content) {
            throw $this->createNotFoundException('Объявление с идентификатором "'.$item_id.'" не найдено');
        }
    ...
    public function contentBySlugAction($slug)
    {
        ...
        $content = $this->getDoctrine()
            ...
            ->createQuery(
                'c',
                function ($qb) use ($rubric, $slug) {
                    $qb->fromRubric($rubric)->whereSlug($slug)->whereEnabled();
                }
            )->getOneOrNullResult();

        if (!$content) {
            throw $this->createNotFoundException('Объявление с кодом "'.$slug.'" не найдено');
        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                try {
                    ...
                    return $this->render(
                        'TooBigAppBundle:Item:add_rate_comment.html.twig',
                        ['form' => $form->createView(), 'item_id' => $item_id]
                    );
                }
            } else {
                try {
    ...
    public function viewItemPhoneAction(Request $request, $item_id)
    {
        ...
        return $this->render(
            'TooBigAppBundle:Captcha:form.html.twig',
            array(
                'form' => $form->createView(),
            )
        );
    ...
    protected function itemsQueryBuilder($rubric, $filter_params, $price_params, $search_params)
    {
        return $this->getDoctrine()
            ...
            ->createQuery(
                'c',
                function ($qb) use ($rubric, $filter_params, $price_params, $search_params) {
                    $qb->fromRubric($rubric)->withSubrubrics(true);
